...
const ServiceCard = ({ name, description, id, icon }) => {
  ...
  return (
    <div
      className="service-card group relative overflow-hidden rounded-xl bg-[var(--bg-surface)] border border-[var(--border-color)] p-8 md:p-9 min-h-[290px] flex flex-col cursor-default"
      tabIndex={0}
    ...
    >
      ...
      <div className="relative z-10 flex flex-col h-full">
        <div className="service-icon-wrap mb-6 flex items-center justify-center w-14 h-14 rounded-lg bg-[var(--bg-elevated)] border border-[var(--border-color)] text-[#FED500]">
          <ServiceIcon type={icon} className="w-7 h-7" />
        </div>

        <h3 className="text-xl md:text-2xl font-bold text-[var(--text-primary)] mb-3">{name}</h3>

        <p className="text-[var(--text-muted)] text-sm leading-relaxed flex-grow min-h-[4.5rem]">
